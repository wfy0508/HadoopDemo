...
public class JobMain extends Configured implements Tool {
    ...
    public int run(String[] strings) throws Exception {
        // 1. 创建一个Job任务对象
        Job job = Job.getInstance(super.getConf(), "mapreduce_dataflow_count_partition");
        job.setJar("HadoopDemo-1.0-SNAPSHOT.jar"); //集群模式

        // 2. 设置Job任务的步骤（共8个步骤）
        // 2.1 指定文件读取使用的类和文件路径
        job.setInputFormatClass(TextInputFormat.class);
        Path inputPath = new Path(hdfsUri + "/dataflow_partition_input");
        ...
        // 2.4（排序）, 2.5（规约）, 2.6（分组）使用默认值
        // 2.7 指定Reducer类和输出键值对
        job.setReducerClass(PartitionReducer.class);
        job.setNumReduceTasks(4); //设置分区个数
        ...
        Path outputPath = new Path(hdfsUri + "/dataflow_partition_output");
